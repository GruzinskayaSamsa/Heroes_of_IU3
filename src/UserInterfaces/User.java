    ...
    public JSONObject getUserJSON() {
        JSONObject user = new JSONObject()
                .put("castle", new JSONObject()
                        .put("units", this.castles.get(0).getArmyJSON()))
                .put("armies", new JSONArray());
        for (int i = 0; i < this.heroes.size(); i++) {
            user.getJSONArray("armies").put(new JSONObject()
                    .put("army", this.heroes.get(i).getArmyJSON())
                    .put("id", this.heroes.get(i).getId()));
        }
